...
export const EditTaskModal = ({
  ...
  const [end, setEnd] = useState<Date | null>(new Date(Date.now() + 60 * 60 * 1000));
  ...
  const handleAddSubtask = useCallback(() => {
    ...
    setSubtasks(prev => [...prev, { _id: Date.now().toString(), text: newSubtask, done: false }]);
    setNewSubtask("");
  }, [newSubtask]);
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      if (!title.trim()) return toast.error("Vui lòng nhập tiêu đề công việc");
      if (!start || !end) return toast.error("Vui lòng chọn thời gian bắt đầu và kết thúc");
      if (end <= start) return toast.error("Thời gian kết thúc phải sau thời gian bắt đầu");

      onEditTask({ title, description, priority, start, end, subtasks });
      toast.success("🎉 Đã sửa công việc thành công!", { duration: 3000 });
      onClose();
    },
    [title, description, priority, start, end, subtasks, onEditTask, onClose]
  );
              ...
              <form onSubmit={handleSubmit} className="relative">
                <div className="sm:grid sm:grid-cols-2 sm:gap-6">
                  ...
                  <div className="space-y-6">
                    <div>
                      <label className="block text-sm text-gray-400 mb-2 ml-1">Tiêu đề công việc</label>
                      <input
                        type="text"
                        value={title}
                    ...
                    <div className="group">
                      <label className="block text-sm text-gray-400 mb-2 ml-1">Mô tả</label>
                ...
                <div className="flex justify-end gap-3 pt-6 mt-auto">
                  <button
                    type="button"
                    onClick={onClose}
                    className="px-4 py-2.5 rounded-xl border border-white/10 text-gray-400 hover:text-white hover:bg-white/10 transition-all duration-200"
                  >
                    Hủy
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 hover:from-blue-500 hover:to-purple-500 transition-all duration-200 flex items-center gap-2"
                  >
